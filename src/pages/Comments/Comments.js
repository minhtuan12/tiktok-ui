...
function Comments() {
    ...
    const videoComment = currentVideo.commentUser ? currentVideo.commentUser : []
    let commentId = videoComment.length > 0 ? videoComment[videoComment.length - 1].commentId++ : 0
    let replyCommentId = videoComment.length > 0 ? videoComment[videoComment.length - 1].reply[videoComment[videoComment.length - 1].reply.length - 1].replyCommentId++ : 0
    ...
    const handlePostComment = () => {
        ...
        if (replyUsername === '') {
            ...
            localStorage.setItem('comment', JSON.stringify(final))
        } else {
// const currentCommentId = videoComment.commentUser
        }
                ...
                <div className={cx('video-info-container')}>
                    <div className={cx('description')}>
                        ...
                        <strong style={{fontWeight: '600'}}> &nbsp;
                            {video[0].tags.map(tag => <><span
                                className={cx('tag')}>#{tag}</span><span>&nbsp;</span></>)}
                        </strong>
                    </div>
